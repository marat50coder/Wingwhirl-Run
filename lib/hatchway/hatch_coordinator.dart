...
class HatchCoordinator {
  ...
  Future<HatchDestination> _decide({
    ...
    assert(() {
      // ignore: avoid_print
      print('[WWR.HATCH] decide start route=${vault.route}');
      return true;
    }());
  ...
  Future<HatchDestination> _firstDecision(
    ...
    if (!await probe.hasInterface()) {
      assert(() {
        // ignore: avoid_print
        print('[WWR.HATCH] first: no interface → offline');
        return true;
      }());
    ...
    if (!await probe.canReachNetwork()) {
      assert(() {
        // ignore: avoid_print
        print('[WWR.HATCH] first: DNS probe failed → offline');
        return true;
      }());
      return const OfflineNest(returnToNative: false);
    }
    ...
    assert(() {
      // ignore: avoid_print
      print(
        '[WWR.HATCH] first: config hasDest=${reply.hasDestination} '
        'url=${reply.url}',
      );
      return true;
    }());
