...
void wwrTrace(String Function() message) {
  assert(() {
    debugPrint(message());
    return true;
  }());
...
class FlightAttribution {
  ...
  Future<void> _start() async {
    ...
    try {
      ...
      wwrTrace(() => '[WWR.FLIGHT] initialization failed: $error');
      _completeEmpty();
    }
  }
  ...
  Future<void> _acceptInstall(dynamic raw) async {
    try {
      ...
      wwrTrace(
        () => '[WWR.FLIGHT] conversion status=$status '
      ...
      wwrTrace(() => '[WWR.FLIGHT] conversion parse error: $error');
      _install = <String, dynamic>{};
    } finally {
  ...
  Future<Map<String, dynamic>> compose({
    ...
    wwrTrace(() => '[WWR.FLIGHT] payload ${jsonEncode(body)}');
    return body;
  }
