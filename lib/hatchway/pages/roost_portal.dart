...
(() => {
  const root = window;
  if (root.__wwrInsetKeeper) return;
  root.__wwrInsetKeeper = true;
  const marker = 'wwr-inset-sheet';
  const rules = [
    ':root{',
...
''');
  ...
  void _installZoomLock() {
    _controller.runJavaScript(r'''
(() => {
  if (window.__wwrZoomLock) return;
  window.__wwrZoomLock = true;
  const lockViewport = () => {
    const host = document.head || document.documentElement;
    if (!host) return;
...
''');
  ...
  void _installTapPolish() {
    _controller.runJavaScript(r'''
(() => {
  if (window.__wwrTapPolish) return;
  window.__wwrTapPolish = true;
  const style = document.createElement('style');
  style.id = 'wwr-tap-polish';
  style.textContent =
...
''');
  ...
  void _installKeyboardLift() {
    _controller.runJavaScript(r'''
(() => {
  if (window.__wwrInputLift) return;
  window.__wwrInputLift = true;
...
''');
  ...
  void _installFocusScaleGuard() {
    ...
    _controller.runJavaScript(r'''
(() => {
  if (window.__wwrFocusScale) return;
  window.__wwrFocusScale = true;
  const style = document.createElement('style');
  style.textContent =
...
''');
  ...
  void _installInlinePlayback() {
    _controller.runJavaScript(r'''
(() => {
  if (window.__wwrInlineMedia) return;
  window.__wwrInlineMedia = true;
  const awaken = (video) => {
